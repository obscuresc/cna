...
#define WINDOWSIZE 6    /* gbn window size = 6 */
...
void A_output(struct msg message)
{
  ...
  if (windowcount < WINDOWSIZE) {
    ...
    /* don't start time if already started, i.e only one packet placed into buffer*/
    if (windowcount == 1) {
      starttimer(A, RTT);
    }
...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (!IsDuplicate) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      /* packet is a new ACK */
      if (TRACE > 0)
      ...
      windowfirst = (windowfirst + ackinc) % WINDOWSIZE;
      windowcount = windowcount - ackinc;

      /***** 1. FILL IN CODE  What else needs to be done when an ACK arrives
       besides removing the packet from the window?  ****/

       /* if still packets in window, stop and restart timer, else stop*/
       stoptimer(A);
       if (windowcount > 0) {
         starttimer(A, RTT);
       }
